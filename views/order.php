...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'create') {
        ...
        if(empty($items)) {
            ...
            $res = $model->createOrderTransaction($_POST['pelanggan_id'], $_POST['meja_id'], $items);
            if ($res === true) {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'bayar') {
        $result = $model->bayarOrder($_POST['id_pesanan'], $_POST['jumlah_bayar'], $_POST['metode']);
...
$activeMenus = $model->getActiveMenus(); 
$pelanggan = $model->getPelanggan();
$meja = $model->getMejaKosong();
$orders = $model->getPendingOrders();
$explain = isset($_GET['analyze']) ? $model->getExplainAnalyze($_GET['analyze']) : "";
?>
